...
class Model
{
    ...
    function insertCsv($chemin,$nomFichier,$colonnes)
    {
        $singleTable = explode(" ", $this->table)[0];

        $info = "testErreurInsert -> ";
//erreur 1 : parenthèse ouvrante supprimée en fin de requête
        $sql = 'LOAD DATA LOCAL INFILE ' . "'" . $chemin . DS . $nomFichier . "'" .  ' INTO TABLE '. $singleTable . " FIELDS TERMINATED BY ',' " ." LINES TERMINATED BY '\\n' IGNORE 1 LINES" ;
        $sql .= ' (';
        $sql .= implode(',', $colonnes);
        $sql .= ')';
        //var_dump($sql);
// résultat du var_dump
// "LOAD DATA LOCAL INFILE '/tmp/php6blF0g/testpersonne.csv' INTO TABLE personne FIELDS TERMINATED BY ',' LINES TERMINATED BY ' ' IGNORE 1 LINES (nom,prenom,age,sexe,mail,adresse)"
// attention ! V2 avec \\r\\n
        try {
